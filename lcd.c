...
void lcd_init(void)
{
  // The LCD uses Port B, bits 0 through 2 for control signals
  // The LCD uses Port D, bits 4 through 7 for data signals

  // Set the DDR register bits for ports B and D
  // configure Port B, bits 0 and 1 for output (0000 0011) - its mask above!
  DDRB |= CTRL_BITS;
  // configure Port D, bits 4 through 7 for output (1111 0000) - its mask above
  DDRD |= DATA_BITS;
...
void lcd_writecommand(unsigned char cmd)
{
  ...
  PORTB &= ~(1 << PB0); // clear register select (PB0) since we are writing command
...
void lcd_writenibble(unsigned char lcdbits)
{
  ...
  // copy lcdbits[7:4] to PORTD[7:4] without touching PORTD[3:0]
  PORTD = (PORTD & ~DATA_BITS) | (lcdbits & DATA_BITS);
